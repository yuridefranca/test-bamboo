...
class RestWrapper
{
  public static function request(string $url, string $method, $formatter = null)
  {
    try {
      $ch = curl_init($url);
...
class Travel
{
  ...
  public function getTravelByCompanyId(string $companyId)
  {
    $travels = $this->list();
    $travels = array_filter((array) $travels, function ($travel) use ($companyId) {
      return $travel->parentId === $companyId;
    });

    return array_values($travels);
  }
  ...
  public function list()
  {
    ...
    $travels = RestWrapper::request($url, $method, function ($response) {
      $travels = json_decode($response, true);
      return array_map(function ($travel) {
        return TravelDTO::fromApiResponse($travel);
      }, $travels);
    });
...
class TravelDTO
{
  ...
  public function __construct(string $createdAt, array $children, string $cost, string $id, string $name, string $parentId)
  {
  ...
  public static function fromApiResponse(array $data)
  {
    return new self(
      ...
      cost: number_format(floatval($data['price']), 2, '.', ''),
      id: $data['id'],
      name: $data['employeeName'],
      parentId: $data['companyId']
    );
  }
}
...
class Company
{
  public function list()
  {
    ...
    $companies = RestWrapper::request($url, $method, function ($response) {
      $companies = json_decode($response, true);
      return array_map(function ($company) {
        $travels = (new Travel())->getTravelByCompanyId($company['id']);
        $company['cost'] = array_reduce($travels, function ($carry, $travel) {
          return $carry + $travel->cost;
        }, 0);

        $company['children'] = $travels;
        return CompanyDTO::fromApiResponse($company);
      }, $companies);
    });
...
class CompanyDTO
{
  ...
  public static function fromApiResponse(array $data)
  {
    return new self(
      createdAt: $data['createdAt'],
      children: $data['children'] ?? [],
      cost: number_format(floatval($data['cost']), 2, '.', '') ?? 0,
      id: $data['id'],
      name: $data['name'],
